...
interface UseBidReturn {
  placeBid: (amount: bigint) => Promise<TransactionResult>
  isLoading: boolean
  error: string | null
  reset: () => void
}
...
/**
 * Hook for placing bids in auction-based circles
 * @param circleAddress - The address of the GyeCircle contract
 */
export function useBid(circleAddress: Address): UseBidReturn {
  ...
  /**
   * Place a bid in the current auction round
   * @param amount - Bid amount in basis points (percentage willing to forfeit)
   */
  ...
  return { placeBid, isLoading, error, reset }
}
