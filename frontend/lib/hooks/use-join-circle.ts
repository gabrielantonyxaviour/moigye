...
interface UseJoinCircleReturn {
  join: (stakeAmount: bigint, contributionAmount: bigint) => Promise<TransactionResult>
  isLoading: boolean
  error: string | null
  reset: () => void
}

export function useJoinCircle(circleAddress: Address): UseJoinCircleReturn {
  ...
  const join = useCallback(
    async (stakeAmount: bigint, contributionAmount: bigint): Promise<TransactionResult> => {
      if (!connectedAddress) {
      ...
      try {
        // Total value = stake + first contribution
        const totalValue = stakeAmount + contributionAmount
  ...
  return { join, isLoading, error, reset }
}
