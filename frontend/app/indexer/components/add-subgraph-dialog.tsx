...
import { getIndexerBaseUrl } from '@/constants/subgraphs'
...
// Default schema for demo - not translated as it's technical content
const DEFAULT_SCHEMA = `type Circle @entity {
  id: ID!
  name: String!
  creator: Bytes!
...
// Get default endpoint from env, fallback to empty string
function getDefaultEndpoint(): string {
  try {
    const baseUrl = getIndexerBaseUrl()
    return `${baseUrl}/subgraphs/name/moigye`
  } catch {
    return ''
  }
}
...
export function AddSubgraphDialog({ open, onOpenChange, provider, onAdd }: AddSubgraphDialogProps) {
  ...
  // Set translated demo values on mount/language change
  useEffect(() => {
    setFormData(prev => ({
      ...prev,
      name: t('demo.subgraph.name'),
      description: t('demo.subgraph.description'),
    }))
  }, [t])
  ...
  const resetForm = () => {
    // Reset to translated demo defaults for easy testing
    setFormData({
      name: t('demo.subgraph.name'),
      slug: 'moigye-circles',
      version: '1.0.0',
